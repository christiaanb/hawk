...
data Virtual r v 
   ...
   | Virtual v (Maybe r)
        deriving (Read,Ord)
instance (Show r, Show v) => Show (Virtual r v) where
    showsPrec _ (Real r) = shows r
    showsPrec _ (Virtual v Nothing) = showString "v" . shows v
    showsPrec _ (Virtual v (Just r)) =
	showString "v" . shows v . showString "{" . shows r . showString "}"
\end{code}
