...
flush k x b s , when b then return x for k cycles, otherwise s <BR>
flush 2 10 <False,False,True,False,False,False,False .. > <1 .. 7 .. > = <1,2,10,10,5,6,7 .. >
\begin{code}
...
fetch (k,translate,max,memory) pc width = (instrs,pc')<BR>
   k = the number of indices between addresses.  <BR>
   translate = the function that creates transaction from
               the representation in memory<BR>
   max = the largest number of instructions to fetch on 
         a single cycle<BR>
   memory = the program<BR>
   pc = the pc to fetch<BR>
   width = the number of instructions to fetch starting at pc<BR>
\begin{code}
...
fetch (k,f,lim,memory@(range,_)) pc n
  ...
  pcs = lift2 (buildPCs k range) pc size
