-- Addressable Queues --
module OA64_Queue(queue,Queue) where
import LazyST
import Maybe
...
type Queue a = (Signal Bool,Signal [a],Signal Int) -> (Signal Int,Signal [a])

queue :: Int -> Queue a
queue n (err,i,k) = unbundle2 $ runST (
  do { q <- new n
--     ; step3(err,i,k) {
     ; loop(bundle3(err,i,k)) $ \ ~(err,i,k) -> do {
          if err then do {clear q; return (n,[])}
           else do { sz <- getSize q
                   ; let k' = min sz k
                   ; out <- mapM (\_ -> deQueue q) [1 .. k']
                   ; sp <- space q
                   ; enList q (take sp i)
                   ; sp <- space q
                   ; return (sp,map fst out)
                   }
       }
     }
