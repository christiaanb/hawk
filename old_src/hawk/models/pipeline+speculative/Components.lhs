...
\begin{code}
fetchDLX x = instrFetch (4 :: Word a => a) dlx2trans x
\end{code}
...
bypassRegFile (instr,writeback)
  = updSrc  instr initSrcValues
    where
      ...
      (regA,regB) = unbundle $ map getSourceRegs instr
...
bypassRF instr wb = updSrc instr vals
   where vals = lift6 build r1 v1 v1' r2 v2 v2'
         ...
         (v1',v2') = unbundle2 $ fmap getSrcVals instr
         (r1,r2) = unbundle2 $ fmap getSrcRegs instr
         ...
         getDstData t = fmap unzip $ fmap (map extractData) $ filterDst isReg t
